...
def calculate_score(a, b):
    ...
    if phone_match(a, b):
        score += 0.85
    score += name_similarity(a, b) * 0.6
    score += company_similarity(a, b) * 0.5
    return min(score, 1.0)
 
def get_match_reasons(a, b):
    """Returns a human-readable list of why two records are flagged as duplicates."""
    reasons = []
    if email_match(a, b):
        reasons.append(f"Same email: {a.get('email')}")
    if phone_match(a, b):
        reasons.append(f"Same phone number")
...
def get_matched_fields(a, b):
    """Returns list of field names that matched."""
    fields = []
    if email_match(a, b):
        fields.append("email")
    if phone_match(a, b):
        fields.append("phone")
    if name_similarity(a, b) >= 0.6:
        fields.append("name")
    if company_similarity(a, b) >= 0.6:
...
def batch_validate_with_ai(pairs, record_map):
    ...
    prompt = "You are a CRM data expert. Determine if each pair is a real duplicate:\n"
...
def run_duplicate_detection(records):
    ...
    for cluster in clusters:
        ...
        # Build reasons from the highest-scoring pair in this cluster
        all_reasons = []
        all_fields = []
        for i in range(len(cluster_records)):
            for j in range(i + 1, len(cluster_records)):
                r = get_match_reasons(cluster_records[i], cluster_records[j])
                f = get_matched_fields(cluster_records[i], cluster_records[j])
        ...
        results.append({
            ...
            # ← NEW: full record details for each contact in the group
            "records": [
                {
                    "id": r["id"],
    ...
    # Sort by confidence descending so highest matches appear first
